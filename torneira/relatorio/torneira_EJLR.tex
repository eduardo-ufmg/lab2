...
O controlador ideal é obtido pela aplicação de um filtro acelerador $f(z) = \frac{1 - p}{z - p}$, com $p = e^{-\frac{2T_s}{\tau}}$, à inversa do componente inversível do sistema. Este é dado por \ref{eq:controlador_ideal}.

\begin{equation}
    \label{eq:controlador_ideal}
    Q(z) = G_-^{-1}(z) f(z) = \frac{1 - 0.997z^{-1}}{-0.004z^{-1}} \frac{0.01}{z - 0.99}
\end{equation}
...
\begin{align}
    ...
    C(z) & = \frac{-1.4 + 2.62z^{-1} - 1.22z^{-2}}{1 - 1.87z^{-1} + 0.88z^{-2} - 0.00z^{-3}} \label{eq:controlador_implementavel}
\end{align}

Sua implementação em equação de diferenças é dada por \ref{eq:controlador_diferencas}.

\begin{align}
    u[n] =\ & -1.4\, e[n] + 2.62\, e[n-1] - 1.22\, e[n-2] \notag \\
           & + 1.87\, u[n-1] - 0.88\, u[n-2] + 0.00\, u[n-3]
