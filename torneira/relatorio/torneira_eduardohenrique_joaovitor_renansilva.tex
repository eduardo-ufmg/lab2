...
Pelas características físicas do sistema, é entendido que a dinâmica térmica da água é dominada pela dinâmica do sensor, ambas de primeira ordem. Assim, o modelo escolhido é o de um sistema de primeira ordem com atraso, cuja função de transferência é dada pela equação \ref{eq:fopdt_simbolico}.:

\begin{equation}
    G(s) = \frac{K e^{-L s}}{\tau s + 1},
    \label{eq:fopdt_simbolico}
\end{equation}

A identificação dos parâmetros é feita a partir de um processo de otimização \cite{2020SciPy-NMeth} para o qual os parametros iniciais são dados pela análise da amplitude de regime permanente, da constante de tempo e do atraso, conforme \cite{Aguirre2007}. O modelo identificado é dado pela equação \ref{eq:fopdt_identificado} e a curva anotada pelos parametros identificados é apresentada na figura \ref{fig:tempos_modelo_tf}.
...
Para filtrar o ruído, assumido gaussiano, nas medições do sistema, um filtro \textit{IIR} de primeira ordem é escolhido, conforme \cite{Lyons2011}, que indica a forma \ref{eq:iir_simbolico}, na qual $\alpha = \exp(-\frac{T_s}{\tau_f})$ é o coeficiente de atenuação, $T_s$ é o período de amostragem e $\tau_f$ é a constante de tempo do filtro.

\begin{equation}
    \hat{y}[k] = \alpha \hat{y}[k-1] + (1 - \alpha) x[k],
    \label{eq:iir_simbolico}
\end{equation}

Seja $\tau_f = L \cdot \text{fator\_atraso}$, em que $\text{fator\_atraso}$ é um hiperparâmetro referente a quanto do atraso do sistema pode ser reintroduzido pelo filtro. Para este trabalho, $\text{fator\_atraso} = 0.15$, correspondente a um atraso total de $115\%$ do atraso original do sistema, proporciona um bom compromisso entre atenuação de ruído e preservação da resposta transitória do sistema.

Uma análise visual das amostras coletadas mostra picos pontuais com duração e amplitude incompatíveis com a dinâmica do sistema. Estas são atribuídas a perturbações no sistema de condicionamento do sinal do sensor de temperatura, não no próprio sistema térmico. Para mitigar o impacto deste tipo de pertubação, \cite{Geoffrine2011} propõe $x[k] = \text{mediana}(y[k], \dots, y[k - m])$, em que $m$ é um hiperparâmetro referente a quantas amostras anteriores devem ser consideradas para o cálculo da mediana. Para este trabalho, $m = 5$ é escolhido, o que corresponde a um tempo de $500 ms$, suficiente para mitigar os picos pontuais sem impactar significativamente a resposta transitória do sistema.

O filtro sintonizado é dado pelo conjunto das equações \ref{eq:iir_sintonizado} e \ref{eq:filtro_mediana}.

\begin{gather}
    \hat{y}[k] = 0.5277 \hat{y}[k-1] + 0.4723 x[k] \label{eq:iir_sintonizado} \\
    x[k] = \text{mediana}(y[k], \dots, y[k - 5]) \label{eq:filtro_mediana}
\end{gather}

A figura \ref{fig:experimento_filtrado} apresenta as curvas de resposta do sistema e do filtro ao par de degraus do experimento.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{../filtro_digital/experimento_filtrado.png}
    \caption{Curvas de resposta do sistema e do filtro ao par de degraus do experimento.}
    \label{fig:experimento_filtrado}
\end{figure}

As figuras \ref{fig:experimento_filtrado_resposta_inicial}, \ref{fig:experimento_filtrado_transitorio} e \ref{fig:experimento_filtrado_estacionario} apresentam as curvas de resposta do sistema e do filtro, respectivamente, para a resposta inicial, a resposta transitória e a resposta em regime permanente.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{../filtro_digital/experimento_filtrado_resposta_inicial.png}
    \caption{Curvas de resposta do sistema e do filtro para a resposta inicial.}
    \label{fig:experimento_filtrado_resposta_inicial}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{../filtro_digital/experimento_filtrado_transitorio.png}
    \caption{Curvas de resposta do sistema e do filtro para a resposta transitória.}
    \label{fig:experimento_filtrado_transitorio}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{../filtro_digital/experimento_filtrado_estacionario.png}
    \caption{Curvas de resposta do sistema e do filtro para a resposta em regime permanente.}
    \label{fig:experimento_filtrado_estacionario}
\end{figure}

\ref{fig:experimento_filtrado} mostra que, para a perturbação mais duradoura, mesmo o sinal filtrado apresenta o perfil de impulso. Foi escolhido não tratar este problema no filtro, afim de preservar a constante de tempo do sistema, importante para o projeto do controlador.

\section{Implementação do Filtro no Sistema e Validação}
